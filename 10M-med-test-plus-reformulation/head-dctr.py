...
#SBATCH --job-name=10M-med-reformulate-test-head-dctr    # Name of the job
#SBATCH --output=head-dctr.out    # Standard output log file
#SBATCH --error=head-dctr.err     # Standard error log file
...
#SBATCH --time=10:00:00                      # Runtime (D-HH:MM:SS format)
...
sys.stdout = open("head-dctr.txt", "w")
...
doc_encoder_path = r"../../trained_model_initial_MedCPT_all/doc_encoder"
query_encoder_path = r"../../trained_model_initial_MedCPT_all/query_encoder"

# Load the document encoder
doc_tokenizer = AutoTokenizer.from_pretrained(r"../../trained_model_initial_MedCPT_all/final_tokenizer")
doc_encoder = AutoModel.from_pretrained(doc_encoder_path)

# Load the query encoder
query_tokenizer = AutoTokenizer.from_pretrained(r"../../trained_model_initial_MedCPT_all/final_tokenizer")
...
# torch.save(query_embeddings, 'query_embeddings_10M_medcpt.pt')

# load the model
query_embeddings = torch.load('query_embeddings_10M_medcpt.pt')
...
# torch.save(document_embeddings, 'document_embeddings_10M_medcpt.pt')

document_embeddings = torch.load('document_embeddings_10M_medcpt.pt')
...
for new_qid in new_query_ids:
    ...
    for doc_id in top_1000_docs:
        if doc_id in dctr_head_queries_test_df['docid'].values:
            qrel_value = dctr_head_queries_test_df[(dctr_head_queries_test_df['qid'] == new_qid) & 
                                                           (dctr_head_queries_test_df['docid'] == doc_id)]['qrel'].values
    ...
    ### test queries
    relevant_docs = dctr_head_queries_test_df[(dctr_head_queries_test_df['qid'] == new_qid) & (dctr_head_queries_test_df['qrel'] != 0)]
...
for new_qid in new_query_ids:
    ...
    for index, row in sim_queries_df.iterrows():
        ...
        sim_qid = row['sim_qid']
        sim_qtext = row['sim_qtext']
        rel_docs = qid_to_rel_docs.get(sim_qid, [])
        ...
        for doc_id in rel_docs:
            if doc_id not in doc_scores:
                doc_scores[doc_id] = 0
            if doc_id in dict(filtered_docs): 
                freq = dict(filtered_docs)[doc_id]
            else:
                freq = 1 
            doc_scores[doc_id] += q_sim_score * log(1 + freq)
        ...
        for docid, freq in aggregate_filtered_docs_2020:
            if docid not in rel_docs:
                if docid not in doc_scores:
                    doc_scores[docid] = 0
                agg_freq = freq
                doc_scores[docid] += q_sim_score * log(1 + agg_freq) * 0.5
    ...
    # Step 3: Compute the augmentation with Reformulation
    # for index, row in sim_queries_df.iterrows():
    #     sim_qid = row['sim_qid']
    #     sim_qtext = row['sim_qtext']
    #     rel_docs = qrel_train_df[(qrel_train_df['qid'] == sim_qid) & (qrel_train_df['qrel'] != 0)]['docid'].dropna().values
    ...
    #     top_5_doc_ids = [docid for docid, _ in sorted_docs[:5]]

    #     unique_combined_docs = list(set(rel_docs + top_5_doc_ids))

    #     for doc_id in unique_combined_docs:
    #         if doc_id not in doc_scores:
    #             doc_scores[doc_id] = 0
    #         doc_scores[doc_id] += q_sim_score
        ...
        #     if docid not in doc_scores:
        #         doc_scores[docid] = 0
        #     doc_scores[docid] += 0.5 * q_sim_score
        ...
        # Filter session_docs to keep only those present in docid
        # filtered_session_docs = [doc for doc in session_docs if doc in docid_set]

        # # Add the log-augmentation score to the document's final score if it is in Rel(q)
        # for doc_id in filtered_session_docs:
        #     if doc_id not in doc_scores:
        #         doc_scores[doc_id] = 0
        #     doc_scores[doc_id] += 0.25 * q_sim_score
...
# Save the dictionary to a file
with open(r'resultsdf_LAplusReformulation_test_head', 'wb') as file:
    pickle.dump(results_df_dict, file)
print("Dictionary saved successfully.")

# Load the dictionary from the file
# with open(r'esultsdf_LAplusReformulation_test_head', 'rb') as file:
#     results_df_dict = pickle.load(file)
# print("Dictionary loaded successfully.")
...
#             if doc_id not in doc_scores:
#                 doc_scores[doc_id] = 0
#             doc_scores[doc_id] += q_sim_score * lambda_val

#         record = data[sim_qid]
#         filtered_docs = [(docid, freq) for docid, freq in record.items() if docid in docid_to_title]
#         sorted_docs = sorted(filtered_docs, key=lambda x: x[1], reverse=True)
#         top_docs_with_titles = sorted_docs[:1]
#         top_doc_ids = [docid for docid, _ in sorted_docs[:1]]

#         # unique_combined_docs = list(set(rel_docs + top_doc_ids))

#         for doc_id in top_doc_ids:
#             if doc_id not in doc_scores:
#                 doc_scores[doc_id] = 0
#             doc_scores[doc_id] += q_sim_score * lambda_val * lambda_val
    ...
    #     q_sim_score = row['q_sim_score']
        ...
        # unique_combined_docs = list(set(rel_docs + session_docs))
        # # Add the log-augmentation score to the document's final score if it is in Rel(q)
        # for doc_id in unique_combined_docs:
        #     if doc_id not in doc_scores:
        #         doc_scores[doc_id] = 0
...
with open(r'resultsdf_DRplusLAplusReformulation_test_head', 'wb') as file:
    pickle.dump(results_df_dict, file)
print("Dictionary saved successfully.")
