...
#SBATCH --job-name=10M-med-test-head-raw    # Name of the job
#SBATCH --output=torso-raw.out    # Standard output log file
#SBATCH --error=torso-raw.err     # Standard error log file
...
sys.stdout = open("torso-raw.txt", "w")
...
new_query_text = torso_queries_test_df['qtext'].tolist()
new_query_ids = torso_queries_test_df['qid'].tolist()
print("torso_queries_test_df.shape[0]", torso_queries_test_df.shape[0])
...
for new_qid, i_query, score_query, i_doc, score_doc in zip(new_query_ids, top_queries_indices, top_queries_similarities, top_documents_indices, top_documents_similarities):
    ...
    # Sort each DataFrame by the similarity scores in descending order
    queries_df_sorted = queries_df.sort_values(by='q_sim_score', ascending=False).reset_index(drop=True)
    documents_df_sorted = documents_df.sort_values(by='doc_sim_score', ascending=False).reset_index(drop=True)

    results_df_dict[f'sim_queries_df_{new_qid}'] = queries_df_sorted
    results_df_dict[f'sim_documents_df_{new_qid}'] = documents_df_sorted
...
with open(r'resultsdf_DR_test_torso', 'wb') as file:
    pickle.dump(results_df_dict, file)
print("Dictionary saved successfully.")


# Load the dictionary from the file
# with open(r'resultsdf_DR_test_torso', 'rb') as file:
...
for new_qid in new_query_ids:
    ...
    for doc_id in top_1000_docs:
        if doc_id in raw_torso_queries_test_df['docid'].values:
            qrel_value = raw_torso_queries_test_df[(raw_torso_queries_test_df['qid'] == new_qid) & 
                                                           (raw_torso_queries_test_df['docid'] == doc_id)]['qrel'].values
    ...
    ### test queries
    relevant_docs = raw_torso_queries_test_df[(raw_torso_queries_test_df['qid'] == new_qid) & (raw_torso_queries_test_df['qrel'] != 0)]
...
# # Save the dictionary to a file
# with open(r'resultsdf_LA_test_torso', 'wb') as file:
#     pickle.dump(results_df_dict, file)
# print("Dictionary saved successfully.")

# # Load the dictionary from the file
# # with open(r'resultsdf_LA_test_torso', 'rb') as file:
# #     results_df_dict = pickle.load(file)
# # print("Dictionary loaded successfully.")
...
for new_qid in new_query_ids:
    ...
    sim_queries_df = results_df_dict[f'sim_queries_df_{new_qid}'].head(10)
    sim_documents_df = results_df_dict[f'sim_documents_df_{new_qid}']
...
with open(r'resultsdf_DRplusLA_test_torso', 'wb') as file:
    pickle.dump(results_df_dict, file)
print("Dictionary saved successfully.")

# Load the dictionary from the file
# with open(r'resultsdf_DRplusLA_test_torso', 'rb') as file:
#     results_df_dict = pickle.load(file)
# print("Dictionary loaded successfully.")
